...
p_tsne_6D <- ggplot(viz_df, aes(tSNE1, tSNE2)) +
    ...
    geom_point(aes(color = label, shape = cl6, alpha = alpha_val), size = 1.9) +
    scale_shape_discrete(name = "Klasteriai") +
...
p_tsne_2D <- ggplot(viz_df, aes(tSNE1, tSNE2)) +
    stat_ellipse(aes(group = cl2, color = cl2),
                 level = 0.90, type = "norm", linetype = 2, linewidth = 1, show.legend = FALSE) +
    geom_point(aes(color = label, shape = cl2, alpha = alpha_val), size = 1.9) +
    scale_shape_discrete(name = "Klasteriai") +
    scale_color_manual(values = class_cols, breaks = names(class_cols), name = "Klasė") +
    scale_alpha_identity() +
...
plot_tsne_linkage <- function(df, cluster_col, title_suffix){
  ggplot(df, aes(tSNE1, tSNE2)) +
    stat_ellipse(aes(group = !!rlang::sym(cluster_col), color = !!rlang::sym(cluster_col)),
                 level = 0.90, type = "norm", linetype = 2, linewidth = 1, show.legend = FALSE) +
    geom_point(aes(color = label, shape = !!rlang::sym(cluster_col), alpha = alpha_val), size = 1.9) +
    scale_shape_discrete(name = "Klasteriai") +
...
p_tsne_31_kbest <- ggplot(viz_df, aes(tSNE1, tSNE2)) +
  ...
  geom_point(aes(color = label, shape = cl31_kbest, alpha = alpha_val), size = 1.9) +
  scale_shape_discrete(name = "Klasteriai") +
  scale_color_manual(values = class_cols, breaks = names(class_cols), name = "Klasė") +
  scale_alpha_identity() +
  labs(
...
p_tsne_31_k4 <- ggplot(viz_df, aes(tSNE1, tSNE2)) +
  stat_ellipse(aes(group = cl31_kvis), level = 0.90,
               type = "norm", linetype = 2, linewidth = 1, color = "grey30") +
  geom_point(aes(color = label, shape = cl31_kvis, alpha = alpha_val), size = 1.9) +
  scale_shape_discrete(name = "Klasteriai") +
  scale_color_manual(values = class_cols, breaks = names(class_cols), name = "Klasė") +
  scale_alpha_identity() +
  labs(
...
p_tsne_2d_k4 <- ggplot(viz_df, aes(tSNE1, tSNE2)) +
  stat_ellipse(aes(group = cl2_kvis), level = 0.90,
               type = "norm", linetype = 2, linewidth = 1, color = "grey30") +
  geom_point(aes(color = label, shape = cl2_kvis, alpha = alpha_val), size = 1.9) +
  scale_shape_discrete(name = "Klasteriai") +
  scale_color_manual(values = class_cols, breaks = names(class_cols), name = "Klasė") +
  scale_alpha_identity() +
  labs(
...
# =========================
# 6D Ward.D2 su išskirčių pašalinimu + t-SNE vizualizacija
# =========================

need <- c("dplyr","ggplot2","cluster","mclust","Rtsne","kableExtra")
to_install <- setdiff(need, rownames(installed.packages()))
if (length(to_install)) install.packages(to_install, dep = TRUE)
invisible(lapply(need, library, character.only = TRUE))

# --- Prielaidos: turi egzistuoti df_mm (su stulpeliu 'label') ir vektorius 'feats' (6 požymiai) ---
stopifnot(exists("df_mm"), exists("feats"))
stopifnot(all(c("label", feats) %in% names(df_mm)))

# A) Pasiruošimas
df_in <- df_mm %>% dplyr::select(label, dplyr::all_of(feats))
X     <- df_in[, feats, drop = FALSE]
y     <- df_in$label

# B) Išskirčių (IQR) žymėjimas per požymius ir eilutes
iqr_flag <- function(x){
    Q1 <- quantile(x, 0.25, na.rm = TRUE); Q3 <- quantile(x, 0.75, na.rm = TRUE)
    I  <- Q3 - Q1
    inner_lo <- Q1 - 1.5*I; inner_hi <- Q3 + 1.5*I
...
out_mat <- sapply(X, iqr_flag)               # stulpinių IQR žymų matrica
row_flag <- apply(out_mat, 1, function(v){   # eilutės lygio žyma: jei bent „Vidinė“ ar „Išorinė“ – šalinti
    if ("Išorinė" %in% v) "Išorinė" else if ("Vidinė" %in% v) "Vidinė" else "Nėra"
})
row_flag <- factor(row_flag, levels = c("Nėra","Vidinė","Išorinė"))

cat("\nIšskirčių suvestinė prieš filtravimą:\n"); print(table(row_flag))

# C) Pašaliname ir vidines, ir išorines išskirtis
keep_idx <- which(row_flag == "Nėra")
df_cln   <- df_in[keep_idx, , drop = FALSE]
X_cln    <- df_cln[, feats, drop = FALSE]
y_cln    <- df_cln$label

cat(sprintf("\nEilučių liko po filtravimo: %d iš %d (%.1f%%)\n",
            nrow(df_cln), nrow(df_in), 100*nrow(df_cln)/nrow(df_in)))

# D) Hierarchinis klasterizavimas 6D erdvėje (Ward.D2, Euclidean)
D6  <- dist(X_cln, method = "euclidean")
hc6 <- hclust(D6, method = "ward.D2")

k <- 4  # <- pasikeisk jei reikia
cl6 <- cutree(hc6, k = k)
...
tab6 <- table(Cluster = cl6, True = y_cln)
sil6 <- sil_mean(cl6, D6)
ari6 <- mclust::adjustedRandIndex(cl6, y_cln)
pur6 <- mean(purity_vec(tab6))

cat("\n--- 6D (Ward.D2) po išskirčių pašalinimo ---\n")
print(tab6)
cat(sprintf("k=%d | Silhouette=%.3f  ARI=%.3f  Purity(avg)=%.3f\n", k, sil6, ari6, pur6))

# (pasirinktinai) atspausdinti kaip gražią lentelę
data.frame(K = k, Silhouette = sil6, ARI = ari6, Purity_avg = pur6) %>%
    kable(digits = 3, caption = "6D Ward.D2 (po išskirčių pašalinimo): metrikos") %>%
    kable_styling(full_width = FALSE)

# E) t-SNE tik vizualizacijai (taškų spalvos = tikros klasės; formos = klasteriai; elipsės = 6D klasteriai)
set.seed(42)  # naudota anksčiau
p_tsne  <- 15
lr_tsne <- 500
ts <- Rtsne(as.matrix(X_cln), dims = 2, perplexity = p_tsne, eta = lr_tsne,
            max_iter = 1000, check_duplicates = FALSE, verbose = FALSE)
...
# (1) t-SNE – klasės spalvomis, su 6D klasterių elipsėmis, klasteriai per shape
p_tsne_classes <- ggplot(viz_df, aes(tSNE1, tSNE2)) +
    stat_ellipse(aes(group = cl6, color = cl6),
                 level = 0.90, type = "norm", linetype = 2, linewidth = 1, show.legend = FALSE) +
    geom_point(aes(color = label, shape = cl6), size = 1.8) +
    scale_shape_manual(
        name   = "Klasteriai",
        breaks = c("1","2","3","4"),
        values = c("1" = 16,   # pilnas apskritimas
                   "2" = 3,   # trikampis
                   "3" = 15,   # kvadratas
                   "4" = 17)    # kryžiukas
    ) +
    scale_color_manual(values = class_cols, breaks = names(class_cols), name = "Klasė") +
    labs(
        title = sprintf("t-SNE (p=%d, lr=%d) – klasės spalvos, 6D Ward.D2 elipsės (k=%d)", p_tsne, lr_tsne, k),
        x = "t-SNE1", y = "t-SNE2"
    ) +
    theme_minimal(base_size = 12) +
    theme(legend.position = "right")

print(p_tsne_classes)
